...
class EditAttackModal(discord.ui.Modal, title="Edit Attack Details"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not (0 <= a1_stars <= 5 and 0 <= a2_stars <= 5):
            await interaction.followup.send(embed=error_embed("Invalid Stars", "Stars must be between 0 and 5."))
            return
        ...
        async with connection.pool.acquire() as conn:
            ...
            if len(attacks) >= 1:
                await conn.execute(
                    """UPDATE attacks SET stars_after = $1, percent_after = $2 
                    WHERE id = $3""",
...
class Attacks(commands.Cog):
    ...
    async def district_autocomplete(self, interaction: discord.Interaction, current: str):
        choices = []
    ...
    @app_commands.autocomplete(match=match_autocomplete, district=district_autocomplete, team=team_autocomplete)
    @is_admin()
    async def log_attack(
        ...
        # Get district number
        district_num = None
        for num, name in DISTRICT_NAMES.items():
        ...
        # Default attacker2 to attacker1
        ...
        async with connection.pool.acquire() as conn:
            ...
            # Check if attacks already exist
            ...
            if existing > 0:
                await interaction.followup.send(
                    embed=error_embed(
                        "Already Logged",
                        f"Attacks for **{district}** have already been logged for **{team}**. Use `/edit-attack` to modify them."
                    )
                )
                return
            ...
            await conn.execute(
                """INSERT INTO attacks (match_id, team_id, district, attack_num, attacker_id, stars_before, percent_before, stars_after, percent_after)
            ...
            await conn.execute(
                ...
                match, team_record['id'], district_num, attacker2.id, attack1_stars, attack1_percent, attack2_stars, attack2_percent
            )
        ...
        await interaction.followup.send(
            embed=success_embed(
                ...
                f"Attack 1: {attacker1.mention} - {attack1_stars}⭐ {attack1_percent}%\n"
                f"Attack 2: {attacker2.mention} - {attack2_stars}⭐ {attack2_percent}%"
            )
        )
    ...
    @app_commands.autocomplete(match=match_autocomplete, district=district_autocomplete, team=team_autocomplete)
    @app_commands.describe(attack_number="Attack number (1 or 2)")
    @is_admin()
    async def edit_attacker(
        ...
        await interaction.followup.send(
            embed=success_embed(
                "Attacker Updated",
                f"Attack #{attack_number} at **{district}** for **{team}** is now assigned to {new_attacker.mention}."
            )
        )
